...
usage(void)
{
  fprintf(stderr,"%s","   [-o outputbasename] [-nsubint nsubint] [-advance adv] [--] infile ...\n");
  fprintf(stderr,"%s","      \n");
  fprintf(stderr,"%s","      Create drift scan beams.\n");
  fprintf(stderr,"%s","      \n");
  fprintf(stderr,"%s","          -o: Basename for the output files\n");
  fprintf(stderr,"%s","              1 char* value\n");
  fprintf(stderr,"%s","    -nsubint: Number of subints per output file\n");
  fprintf(stderr,"%s","              1 int value\n");
  fprintf(stderr,"%s","    -advance: Number of subints to advance for next file\n");
  fprintf(stderr,"%s","              1 int value\n");
  fprintf(stderr,"%s","      infile: Input file name(s) of the PSRFITs datafiles\n");
  fprintf(stderr,"%s","              1...2000 values\n");
  fprintf(stderr,"%s","  version: 18Nov13\n");
  fprintf(stderr,"%s","  ");
  exit(EXIT_FAILURE);
}
/**********************************************************************/
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-nsubint", argv[i]) ) {
      int keep = i;
      cmd.nsubintP = 1;
      i = getIntOpt(argc, argv, i, &cmd.nsubint, 1);
      cmd.nsubintC = i-keep;
      continue;
    }

    if( 0==strcmp("-advance", argv[i]) ) {
      int keep = i;
      cmd.advP = 1;
      i = getIntOpt(argc, argv, i, &cmd.adv, 1);
      cmd.advC = i-keep;
      continue;
    }
